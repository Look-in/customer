...
<div>
    ...
    <c:forEach var="typeitem" items="${typeitem}">
        <c:url var="selectitem" value="selectitemservlet">
            <c:param name="action" value="list"/>
            <c:param name="type" value="${typeitem.value}"/>
            <c:param name="typeId" value="${typeitem.key}"/>
        </c:url>
        <a href="${selectitem}" title="">${typeitem.value}</a>
    </c:forEach>
</div>
<div class="add-item">
...
<div class="add-item">
    <form name="selecting" method="get" action="#">
        ...
        <select name="sortingBy">
            ...
            <c:forEach var="sort" items="${sortBy}">
                <option value="${sort}" ${param.sortingBy == sort ? 'selected="selected"' : ''}>${sort}</option>
            </c:forEach>
        </select>
